本项目中使用的液晶屏主控为 st7789，分辨率 240x240，图像格式 RGB565。
...
说它在意料之内是因为这些物理寄存器都是通过硬件置位的，以发送 FIFO 为空标志为例，当 SPI 发送完成瞬间它就会被硬件置位，这一点在任何一款单片机上都可以验证，实际操作一下就会发现类似的寄存器是瞬间被置位的。pl022 应该是非常成熟的 SPI 控制器，我觉得芯片设计人员不会犯发送 FIFO 为空后很长时间才设置标志位这种低级错误。
