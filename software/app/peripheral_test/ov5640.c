...
static uint8_t OV5640_wreg(I2CDevice *i2c_dev, uint16_t addr, uint8_t data)
{
    uint8_t buf[4] = {data, 0, 0, 0};
    i2c_ioctl_write(i2c_dev, addr, buf, 1);
    return 0;
}

static uint8_t OV5640_rreg(I2CDevice *i2c_dev, uint16_t addr)
{
    uint8_t buf[4] = {0, 0, 0, 0};
    i2c_ioctl_read(i2c_dev, addr, buf, 1);
    return buf[0];
}
...
int32_t OV5640_Init(I2CDevice *i2c_dev)
{
    ...
    reg = OV5640_rreg(i2c_dev, 0x300A);
    if (reg != 0x56) {
        printf("sensor id1 0x%02x is not 0x56\n", reg);
        return 1;
    }
    reg = OV5640_rreg(i2c_dev, 0x300B);
    if (reg != 0x40) {
        printf("sensor id2 0x%02x is not 0x40\n", reg);
        return 1;
    }

    
    OV5640_wreg(i2c_dev, 0x3103, 0x11); // SCCB system control
    OV5640_wreg(i2c_dev, 0x3008, 0x82); // software reset
    sleep_ms(5);

    for (i = 0; i < sizeof(ov5640_init_settings) / sizeof(ov5640_init_settings[0]); i++) {
        OV5640_wreg(i2c_dev, ov5640_init_settings[i].addr, ov5640_init_settings[i].value);
    }
    sleep_ms(5);

    for (i = 0; i < sizeof(ov5640_vga_30fps_settings) / sizeof(ov5640_vga_30fps_settings[0]); i++) {
        OV5640_wreg(i2c_dev, ov5640_vga_30fps_settings[i].addr, ov5640_vga_30fps_settings[i].value);
    }
