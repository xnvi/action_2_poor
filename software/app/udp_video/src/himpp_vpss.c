...
HI_S32 HIMPP_VPSS_Start(VPSS_GRP VpssGrp, HI_BOOL* pabChnEnable, VPSS_GRP_ATTR_S* pstVpssGrpAttr, VPSS_CHN_ATTR_S* pastVpssChnAttr)
{
    ...
    for (j = 0; j < VPSS_MAX_PHY_CHN_NUM; j++)
    {
        if(HI_TRUE == pabChnEnable[j])
        {
            ...
            if (s32Ret != HI_SUCCESS)
            {
                HIMPP_PRT("HI_MPI_VPSS_SetChnAttr failed with %#x\n", s32Ret);
                return HI_FAILURE;
            }
            ...
            if (s32Ret != HI_SUCCESS)
            {
                HIMPP_PRT("HI_MPI_VPSS_EnableChn failed with %#x\n", s32Ret);
                return HI_FAILURE;
            }
        }
    }
    ...
    if (s32Ret != HI_SUCCESS)
    {
        HIMPP_PRT("HI_MPI_VPSS_StartGrp failed with %#x\n", s32Ret);
        return HI_FAILURE;
    }
    ...
    for (j = 0; j < VPSS_MAX_PHY_CHN_NUM; j++)
    {
        if(HI_TRUE == pabChnEnable[j])
        {
            ...
            if (VPSS_CHN0 == VpssChn)
            {
                ...
                if (s32Ret != HI_SUCCESS)
                {
                    HIMPP_PRT("HI_MPI_VPSS_SetChnBufWrapAttr failed with %#x\n", s32Ret);
                    return HI_FAILURE;
                }
            }
