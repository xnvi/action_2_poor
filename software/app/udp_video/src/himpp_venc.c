...
static HIMPP_VENC_GETSTREAM_PARA_S gs_stPara;
static HIMPP_VENC_QPMAP_SENDFRAME_PARA_S stQpMapSendFramePara;
...
******************************************************************************/
HI_S32 HIMPP_VENC_GetFilePostfix(PAYLOAD_TYPE_E enPayload, char* szFilePostfix)
{
...
HI_S32 HIMPP_VENC_GetGopAttr(VENC_GOP_MODE_E enGopMode,VENC_GOP_ATTR_S *pstGopAttr)
{
    switch(enGopMode)
    {
        ...
        default:
            HIMPP_PRT("not support the gop mode !\n");
            return HI_FAILURE;
        break;
    }
    return HI_SUCCESS;
...
HI_S32 HIMPP_VENC_Getdcfinfo(char* SrcJpgPath, char* DstThmPath)
{
...
******************************************************************************/
HI_S32 HIMPP_VENC_SaveStream(FILE* pFd, VENC_STREAM_S* pstStream)
{
    HI_S32 i;
...
******************************************************************************/
HI_S32 HIMPP_VENC_SaveStream_PhyAddr(FILE* pFd, VENC_STREAM_BUF_INFO_S *pstStreamBuf, VENC_STREAM_S* pstStream)
{
    HI_U32 i,j;
...
HI_S32 HIMPP_VENC_CloseReEncode(VENC_CHN VencChn)
{
    HI_S32 s32Ret;
...
HI_S32 HIMPP_VENC_Creat(VENC_CHN VencChn, PAYLOAD_TYPE_E enType,  PIC_SIZE_E enSize, HIMPP_RC_E enRcMode, HI_U32  u32Profile, HI_BOOL bRcnRefShareBuf,VENC_GOP_ATTR_S *pstGopAttr)
{
    ...
    HIMPP_VI_CONFIG_S     stViConfig;
    HI_U32                 u32FrameRate;
    HI_U32                 u32StatTime;
    HI_U32                 u32Gop = 30;

    s32Ret = HIMPP_SYS_GetPicSize( enSize, &stPicSize);
    if (HI_SUCCESS != s32Ret)
    {
        HIMPP_PRT("Get picture size failed!\n");
        return HI_FAILURE;
    }
    ...
    s32Ret = HIMPP_VI_GetFrameRateBySensor(stViConfig.astViInfo[0].stSnsInfo.enSnsType, &u32FrameRate);
    if (HI_SUCCESS != s32Ret)
    {
        HIMPP_PRT("HIMPP_VI_GetFrameRateBySensor failed!\n");
        return s32Ret;
    }
    ...
    ******************************************/
    ...
    switch (enType)
    {
        case PT_H265:
        {
            ...
            else if (HIMPP_RC_FIXQP == enRcMode)
            {
            ...
            else if(HIMPP_RC_AVBR == enRcMode)
            {
            ...
            else if(HIMPP_RC_QVBR == enRcMode)
            {
            ...
            else if(HIMPP_RC_CVBR == enRcMode)
            {
            ...
            else if(HIMPP_RC_QPMAP == enRcMode)
            {
            ...
            {
                HIMPP_PRT("%s,%d,enRcMode(%d) not support\n",__FUNCTION__,__LINE__,enRcMode);
                return HI_FAILURE;
            }
        ...
        case PT_MJPEG:
        {
            ...
            {
                HIMPP_PRT("cann't support other mode(%d) in this version!\n",enRcMode);
                return HI_FAILURE;
            }
        }
        break;
        ...
        case PT_JPEG:
            ...
            break;
        default:
            HIMPP_PRT("cann't support this enType (%d) in this version!\n",enType);
    ...
    else
    {
        ...
        if((VENC_GOPMODE_BIPREDB == pstGopAttr->enGopMode)&&(PT_H264 == enType))
        {
            if(0 == stVencChnAttr.stVencAttr.u32Profile)
            {
                ...
                HIMPP_PRT("H.264 base profile not support BIPREDB, so change profile to main profile!\n");
            }
        }
        ...
        if((VENC_RC_MODE_H264QPMAP == stVencChnAttr.stRcAttr.enRcMode)||(VENC_RC_MODE_H265QPMAP == stVencChnAttr.stRcAttr.enRcMode))
        {
            if(VENC_GOPMODE_ADVSMARTP == pstGopAttr->enGopMode)
            {
                ...
                HIMPP_PRT("advsmartp not support QPMAP, so change gopmode to smartp!\n");
            }
        }
    }
    ...
    if (HI_SUCCESS != s32Ret)
    {
        HIMPP_PRT("HI_MPI_VENC_CreateChn [%d] faild with %#x! ===\n", \
                   VencChn, s32Ret);
        return s32Ret;
    }

    s32Ret = HIMPP_VENC_CloseReEncode(VencChn);
    if (HI_SUCCESS != s32Ret)
    {
...
HI_S32 HIMPP_VENC_Start(VENC_CHN VencChn, PAYLOAD_TYPE_E enType,  PIC_SIZE_E enSize, HIMPP_RC_E enRcMode, HI_U32  u32Profile, HI_BOOL bRcnRefShareBuf,VENC_GOP_ATTR_S *pstGopAttr)
{
    ...
    ******************************************/
    s32Ret = HIMPP_VENC_Creat(VencChn,enType,enSize,enRcMode,u32Profile,bRcnRefShareBuf,pstGopAttr);
    if (HI_SUCCESS != s32Ret)
    {
        HIMPP_PRT("HIMPP_VENC_Creat faild with%#x! \n", s32Ret);
        return HI_FAILURE;
    }
    /******************************************
    ...
    ******************************************/
    ...
    if (HI_SUCCESS != s32Ret)
    {
        HIMPP_PRT("HI_MPI_VENC_StartRecvPic faild with%#x! \n", s32Ret);
        return HI_FAILURE;
    }
    return HI_SUCCESS;
}
...
HI_S32 HIMPP_VENC_Stop(VENC_CHN VencChn)
{
    ...
    ******************************************/
    ...
    if (HI_SUCCESS != s32Ret)
    {
        HIMPP_PRT("HI_MPI_VENC_StopRecvPic vechn[%d] failed with %#x!\n", \
                   VencChn, s32Ret);
        return HI_FAILURE;
    }    /******************************************
    ...
    ******************************************/
    ...
    if (HI_SUCCESS != s32Ret)
    {
        HIMPP_PRT("HI_MPI_VENC_DestroyChn vechn[%d] failed with %#x!\n", \
                   VencChn, s32Ret);
        return HI_FAILURE;
    }
    return HI_SUCCESS;
...
HI_S32 HIMPP_VENC_SnapProcess(VENC_CHN VencChn, HI_U32 SnapCnt, HI_BOOL bSaveJpg, HI_BOOL bSaveThm)
{
    ...
    ******************************************/
    ...
    if (s32Ret == HI_SUCCESS && stSrcChn.enModId == HI_ID_VPSS)
    {
        ...
        if (s32Ret == HI_SUCCESS && stVpssChnAttr.enCompressMode != COMPRESS_MODE_NONE)
        {
            ...
            {
                HIMPP_PRT("call HI_MPI_VPSS_TriggerSnapFrame Grp = %d, ChanId = %d, SnapCnt = %d return failed(0x%x)!\n",
    ...
    ******************************************/
    ...
    if (s32VencFd < 0)
    {
        HIMPP_PRT("HI_MPI_VENC_GetFd faild with%#x!\n", s32VencFd);
        return HI_FAILURE;
    }
    ...
    for(i=0; i<SnapCnt; i++)
    {
        ...
        if (s32Ret < 0)
        {
            HIMPP_PRT("snap select failed!\n");
            return HI_FAILURE;
        }
        else if (0 == s32Ret)
        {
            HIMPP_PRT("snap time out!\n");
            return HI_FAILURE;
        }
        else
        {
            if (FD_ISSET(s32VencFd, &read_fds))
            {
                ...
                {
                    HIMPP_PRT("malloc memory failed!\n");
                    return HI_FAILURE;
                }
                ...
                if (HI_SUCCESS != s32Ret)
                {
                    HIMPP_PRT("HI_MPI_VENC_GetStream failed with %#x!\n", s32Ret);
                ...
                if(bSaveJpg || bSaveThm)
                {
                    ...
                    s32Ret = HIMPP_VENC_SaveStream(pFile, &stStream);
                    if (HI_SUCCESS != s32Ret)
                    {
                        HIMPP_PRT("save snap picture failed!\n");
                    ...
                    if (HI_SUCCESS != s32Ret)
                    {
                        HIMPP_PRT("HI_MPI_VENC_GetStreamBufInfo failed with %#x!\n", s32Ret);
                    ...
                    s32Ret = HIMPP_VENC_SaveStream_PhyAddr(pFile, &stStreamBufInfo, &stStream);
                    if (HI_SUCCESS != s32Ret)
                    {
                        HIMPP_PRT("save snap picture failed!\n");
                    ...
                    if(bSaveThm)
                    {
                        ...
                        s32Ret = HIMPP_VENC_Getdcfinfo(acFile, acFile_dcf);
                        if (HI_SUCCESS != s32Ret)
                        {
                            HIMPP_PRT("save thm picture failed!\n");
                ...
                if (HI_SUCCESS != s32Ret)
                {
                    HIMPP_PRT("HI_MPI_VENC_ReleaseStream failed with %#x!\n", s32Ret);
    ...
    ******************************************/
    ...
    if (s32Ret != HI_SUCCESS)
    {
        HIMPP_PRT("HI_MPI_VENC_StopRecvPic failed with %#x!\n",  s32Ret);
        return HI_FAILURE;
    }
    return HI_SUCCESS;
}
...
HI_VOID* HIMPP_QpmapSendFrameProc(HI_VOID* p)
{
    HI_U32 i,j,FrmId;
    HI_S32 s32Ret;
    VIDEO_FRAME_INFO_S  *pstVideoFrame;
    USER_FRAME_INFO_S   stFrame[VENC_QPMAP_MAX_CHN][QpMapBufNum];
    HIMPP_VENC_QPMAP_SENDFRAME_PARA_S *pstPara;
    ...
    pstPara = (HIMPP_VENC_QPMAP_SENDFRAME_PARA_S*)p;

    if (pstPara->s32Cnt > VENC_QPMAP_MAX_CHN)
    {
        HIMPP_PRT("Current func'HIMPP_QpmapSendFrameProc' not support Venc channal num(%d) > %d\n",pstPara->s32Cnt,VENC_QPMAP_MAX_CHN);
        return NULL;
    }
    ...
    for (i = 0; i < pstPara->s32Cnt; i++)
    {
        ...
        if (HI_SUCCESS != s32Ret)
        {
            HIMPP_PRT("HI_MPI_SYS_MmzAlloc err:0x%x",s32Ret);
            goto error;
        }
    ...
    for (i = 0; i < pstPara->s32Cnt; i++)
    {
        ...
        if (HI_SUCCESS != s32Ret)
        {
            HIMPP_PRT("HI_MPI_VPSS_GetChnAttr err:0x%x",s32Ret);
            goto error;
        }
        ...
        if (HI_SUCCESS != s32Ret)
        {
            HIMPP_PRT("HI_MPI_VPSS_SetChnAttr err:0x%x",s32Ret);
            goto error;
        }
    }
    ...
    while(HI_TRUE == pstPara->bThreadStart)
    {
        for (i = 0; i < pstPara->s32Cnt; i++)
        {
            ...
            if (HI_SUCCESS != s32Ret)
            {
                HIMPP_PRT("HI_MPI_VPSS_GetChnFrame err:0x%x\n",s32Ret);
                continue;
            }
            ...
            if(HI_SUCCESS != s32Ret)
            {
                HIMPP_PRT("HI_MPI_VENC_SendFrame err:0x%x\n",s32Ret);

                s32Ret = HI_MPI_VPSS_ReleaseChnFrame(pstPara->VpssGrp,pstPara->VpssChn[i],pstVideoFrame);
                if(HI_SUCCESS != s32Ret)
                {
                    HIMPP_PRT("HI_MPI_VPSS_ReleaseChnFrame err:0x%x",s32Ret);
                    goto error;
                }
                break;
            }
...
error:
    for (i = 0; i < pstPara->s32Cnt; i++)
    {
        if (u64QpMapPhyAddr[i][0] != 0)
        {
            ...
            if(HI_SUCCESS != s32Ret)
            {
                HIMPP_PRT("HI_MPI_SYS_MmzFree err:0x%x",s32Ret);
            }
        }
...
HI_S32 HIMPP_VENC_QpmapSendFrame(VPSS_GRP VpssGrp,VPSS_CHN VpssChn[],VENC_CHN VeChn[],HI_S32 s32Cnt,SIZE_S stSize[])
{
    ...
    return pthread_create(&gs_VencQpmapPid, 0, HIMPP_QpmapSendFrameProc, (HI_VOID*)&stQpMapSendFramePara);
}
...
******************************************************************************/
HI_VOID* HIMPP_VENC_GetVencStreamProc(HI_VOID* p)
{
    HI_S32 i;
    HI_S32 s32ChnTotal;
    VENC_CHN_ATTR_S stVencChnAttr;
    HIMPP_VENC_GETSTREAM_PARA_S* pstPara;
    HI_S32 maxfd = 0;
    struct timeval TimeoutVal;
    fd_set read_fds;
    ...
    pstPara = (HIMPP_VENC_GETSTREAM_PARA_S*)p;
    ...
    ******************************************/
    ...
    for (i = 0; i < s32ChnTotal; i++)
    {
        ...
        if (s32Ret != HI_SUCCESS)
        {
            HIMPP_PRT("HI_MPI_VENC_GetChnAttr chn[%d] failed with %#x!\n", \
                       VencChn, s32Ret);
            return NULL;
        }
        enPayLoadType[i] = stVencChnAttr.stVencAttr.enType;

        s32Ret = HIMPP_VENC_GetFilePostfix(enPayLoadType[i], szFilePostfix);
        if (s32Ret != HI_SUCCESS)
        {
            HIMPP_PRT("HIMPP_VENC_GetFilePostfix [%d] failed with %#x!\n", \
        ...
        if (VencFd[i] < 0)
        {
            HIMPP_PRT("HI_MPI_VENC_GetFd failed with %#x!\n",
                       VencFd[i]);
            return NULL;
        }
        if (maxfd <= VencFd[i])
    ...
    ******************************************/
    while (HI_TRUE == pstPara->bThreadStart)
    {
        ...
        if (s32Ret < 0)
        {
            HIMPP_PRT("select failed!\n");
            break;
        }
        else if (s32Ret == 0)
        {
            HIMPP_PRT("get venc stream time out, exit thread\n");
            continue;
        }
        else
        {
            for (i = 0; i < s32ChnTotal; i++)
            {
                if (FD_ISSET(VencFd[i], &read_fds))
                {
                    ...
                    *******************************************************/
                    ...
                    if (HI_SUCCESS != s32Ret)
                    {
                        HIMPP_PRT("HI_MPI_VENC_QueryStatus chn[%d] failed with %#x!\n", VencChn, s32Ret);
                        break;
                    }
                    ...
                    *******************************************************/
                    ...
                    if (NULL == stStream.pstPack)
                    {
                        HIMPP_PRT("malloc stream pack failed!\n");
                        break;
                    }
                    ...
                    *******************************************************/
                    ...
                    if (HI_SUCCESS != s32Ret)
                    {
                        HIMPP_PRT("HI_MPI_VENC_ReleaseStream failed!\n");
                        free(stStream.pstPack);
                        stStream.pstPack = NULL;
                        break;
                    }
...
HI_S32 HIMPP_VENC_StartGetStream(VENC_CHN VeChn[],HI_S32 s32Cnt)
{
    ...
    return pthread_create(&gs_VencPid, 0, HIMPP_VENC_GetVencStreamProc, (HI_VOID*)&gs_stPara);
}
...
HI_VOID HIMPP_VENC_ReadOneFrame( FILE* fp, HI_U8* pY, HI_U8* pU, HI_U8* pV,
...
HI_S32 HIMPP_VENC_PlanToSemi(HI_U8* pY, HI_S32 yStride,
