...
#define HIMPP_MEM_DEV_OPEN if (s_s32SampleMemDev <= 0)\
{\
...
HI_S32 HIMPP_SYS_SetReg(HI_U64 u64Addr, HI_U32 u32Value)
{
    ...
    pu32RegAddr = (HI_U32 *)HIMPP_SYS_IOMmap(u64Addr, u32MapLen);
    if(NULL == pu32RegAddr)
    {
        return HI_FAILURE;
    }
    ...
    *pu32RegAddr = u32Value;
    ...
    return HIMPP_SYS_Munmap(pu32RegAddr, u32MapLen);
}
...
HI_S32 HIMPP_SYS_GetPicSize(PIC_SIZE_E enPicSize, SIZE_S* pstSize)
{
    ...
    {
		case PIC_PREVIEW:
            pstSize->u32Width  = preview_u32Width;
            pstSize->u32Height = preview_u32Height;
            break;
...
HI_S32 HIMPP_SYS_MemConfig(HI_VOID)
{
    ...
    for (i = 0; i < VI_MAX_PIPE_NUM; i++)
    {
        for (j = 0; j < VI_MAX_CHN_NUM; j++)
        {
            ...
            if (s32Ret)
            {
                HIMPP_PRT("HI_MPI_SYS_SetMemConfig ERR !\n");
                return HI_FAILURE;
            }
        }
    }
    ...
    for (i = 0; i < VENC_MAX_CHN_NUM; i++)
    {
        ...
        if (s32Ret)
        {
            HIMPP_PRT("HI_MPI_SYS_SetMemConf ERR !\n");
            return HI_FAILURE;
        }
    }
...
HI_S32 HIMPP_SYS_Init(VB_CONFIG_S* pstVbConfig)
{
    ...
    if (NULL == pstVbConfig)
    {
        HIMPP_PRT("input parameter is null, it is invaild!\n");
        return HI_FAILURE;
    }
    ...
    {
        HIMPP_PRT("HI_MPI_VB_SetConf failed!(%#x)\n", s32Ret);
        return HI_FAILURE;
    }
    ...
    if (HI_SUCCESS != s32Ret)
    {
        HIMPP_PRT("HI_MPI_VB_Init failed!(%#x)\n", s32Ret);
        return HI_FAILURE;
    }
    ...
    if (HI_SUCCESS != s32Ret)
    {
        HIMPP_PRT("HI_MPI_SYS_Init failed!(%#x)\n", s32Ret);
        HI_MPI_VB_Exit();
        return HI_FAILURE;
    }
...
HI_S32 HIMPP_SYS_InitWithVbSupplement(VB_CONFIG_S* pstVbConf, HI_U32 u32SupplementConfig)
{
    ...
    if (HI_SUCCESS != s32Ret)
    {
        HIMPP_PRT("HI_MPI_VB_SetConf failed!\n");
        return HI_FAILURE;
    }
    ...
    if (HI_SUCCESS != s32Ret)
    {
        HIMPP_PRT("HI_MPI_VB_SetSupplementConf failed!\n");
        return HI_FAILURE;
    }
    ...
    if (HI_SUCCESS != s32Ret)
    {
        HIMPP_PRT("HI_MPI_VB_Init failed!\n");
        return HI_FAILURE;
    }
    ...
    if (HI_SUCCESS != s32Ret)
    {
        HIMPP_PRT("HI_MPI_SYS_Init failed!\n");
        return HI_FAILURE;
    }
...
HI_S32 HIMPP_AVS_Bind_AVS(AVS_GRP AvsSrcGrp, AVS_CHN AvsSrcChn, AVS_GRP AvsDestGrp, AVS_CHN AvsDestChn)
{
    MPP_CHN_S stSrcChn;
    MPP_CHN_S stDestChn;
...
HI_S32 HIMPP_AVS_UnBind_AVS(AVS_GRP AvsSrcGrp, AVS_CHN AvsSrcChn, AVS_GRP AvsDestGrp, AVS_CHN AvsDestChn)
{
    MPP_CHN_S stSrcChn;
    MPP_CHN_S stDestChn;
...
HI_S32 HIMPP_AVS_Bind_VPSS(AVS_GRP AvsGrp, AVS_CHN AvsChn, VPSS_GRP VpssGrp)
{
    MPP_CHN_S stSrcChn;
    MPP_CHN_S stDestChn;
...
HI_S32 HIMPP_AVS_UnBind_VPSS(AVS_GRP AvsGrp, AVS_CHN AvsChn, VPSS_GRP VpssGrp)
{
    MPP_CHN_S stSrcChn;
    MPP_CHN_S stDestChn;
...
HI_S32 HIMPP_AVS_Bind_VENC(AVS_GRP AvsGrp, AVS_CHN AvsChn, VENC_CHN VencChn)
{
    MPP_CHN_S stSrcChn;
    MPP_CHN_S stDestChn;
...
HI_S32 HIMPP_AVS_UnBind_VENC(AVS_GRP AvsGrp, AVS_CHN AvsChn, VENC_CHN VencChn)
{
    MPP_CHN_S stSrcChn;
    MPP_CHN_S stDestChn;
...
HI_S32 HIMPP_AVS_Bind_VO(AVS_GRP AvsGrp, AVS_CHN AvsChn, VO_LAYER VoLayer, VO_CHN VoChn)
{
    MPP_CHN_S stSrcChn;
    MPP_CHN_S stDestChn;
...
HI_S32 HIMPP_AVS_UnBind_VO(AVS_GRP AvsGrp, AVS_CHN AvsChn, VO_LAYER VoLayer, VO_CHN VoChn)
{
    MPP_CHN_S stSrcChn;
    MPP_CHN_S stDestChn;
...
HI_S32 HIMPP_VDEC_Bind_VPSS(VDEC_CHN VdecChn, VPSS_GRP VpssGrp)
{
    MPP_CHN_S stSrcChn;
    MPP_CHN_S stDestChn;
...
HI_S32 HIMPP_VDEC_UnBind_VPSS(VDEC_CHN VdecChn, VPSS_GRP VpssGrp)
{
    MPP_CHN_S stSrcChn;
    MPP_CHN_S stDestChn;
