...
static HI_S32 HIMPP_VENC_VPSS_Init(VPSS_GRP VpssGrp, HIMPP_VPSS_CHN_ATTR_S *pstParam)
{
    ...
    // my edit 视频预览的缩略图通道，特殊设置
    // 设置裁剪属性，必须在 StartGrp 之后
    VPSS_CROP_INFO_S stVpssCropAttr;
    stVpssCropAttr.bEnable = HI_TRUE;
    stVpssCropAttr.enCropCoordinate = VPSS_CROP_ABS_COOR;
    stVpssCropAttr.stCropRect.s32X = 1920 - preview_crop_u32Width;
    stVpssCropAttr.stCropRect.s32Y = 0;
    stVpssCropAttr.stCropRect.u32Width = preview_crop_u32Width;
    stVpssCropAttr.stCropRect.u32Height = preview_crop_u32Height;
    HI_MPI_VPSS_SetChnCrop(VpssGrp, 4, &stVpssCropAttr);
