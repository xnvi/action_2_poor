...
static lv_color_t lcd_buf1[LCD_SCREEN_WIDTH * LCD_SCREEN_HEIGHT / 2];
static lv_color_t lcd_buf2[LCD_SCREEN_WIDTH * LCD_SCREEN_HEIGHT / 2];
...
void my_disp_flush(lv_disp_drv_t * disp, const lv_area_t * area, lv_color_t * color_p)
{
    ...
    lcd_data.area.len = (area->x2 - area->x1) * (area->y2 - area->y1) * sizeof(lv_color_t);
...
void *pth_lvgl_func(void *args)
{
	...
	memset(lcd_buf1, 0, sizeof(lcd_buf1));
	memset(lcd_buf2, 0, sizeof(lcd_buf2));
	lv_disp_draw_buf_init(&lvgl_disp_buf, lcd_buf1, lcd_buf2, LCD_SCREEN_WIDTH * LCD_SCREEN_HEIGHT);
